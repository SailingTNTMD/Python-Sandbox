...
Goal:
      ...
      intersect: The keys to the intersect dictionary are keys that are common in both
      d1 and d2. To get the values of the intersect dictionary, look at the common keys
      in d1 and d2 and apply the function f to these keys' values --the value of the
      common key in d1 is the first parameter to the function and the value of the common
      key in d2 is the second parameter to the function.
...
def dict_interdiff(d1, d2):
      ...
      dictInter = {}
      dictDiff = {}
      tupleEval = (dictInter, dictDiff)
      
      for i in d1:
            if i in d2:
                  dictInter[i] = f(d1[i], d2[i])
            elif i not in d2:
                  dictDiff[i] = d1[i]
